...
def render_do_payment():
            ...
            if not payment_token:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        st.session_state['response_summary'] = f"✅ Request successful in {duration}s"
                    else:
    ...
    with col2:
        ...
        if st.session_state.get('submitted'):
            ...
            duration_match = re.search(r"in (\d+\.\d+)s", summary)
            duration_text = f"⏱ {duration_match.group(1)}s" if duration_match else ""
            st.info(f"{summary} {duration_text}")
            ...
            if match:
                try:
                    ...
                    if isinstance(redirect_url, str) and redirect_url.startswith("http"):
                        st.markdown(f"[➡️ Click here to proceed]({redirect_url})", unsafe_allow_html=True)
                except Exception:
