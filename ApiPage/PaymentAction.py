...
def render_payment_action():
    ...
    # File uploads with enhanced error handling
    ...
    # Process files only if both are uploaded successfully
    private_jwk = None
    public_jwk = None
    ...
    if private_key_file is not None and public_cert_file is not None:
        try:
            ...
            if private_key_file.name.lower().endswith((".pfx", ".p12")):
                pfx_data = private_key_file.getvalue()
                ...
                pem_data = private_key_file.getvalue()
            ...
            st.info("💡 Tips:")
            st.info("- Ensure password is correct")
            st.info("- Check if files are not corrupted")
            st.info("- Try re-uploading the files")
            private_jwk = None
            public_jwk = None
    ...
    if private_jwk is not None and public_jwk is not None:
        ...
        st.warning("⚠️ Please upload and verify both private key and public certificate files first.")
        send_request = False
